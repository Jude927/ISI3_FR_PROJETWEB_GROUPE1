...
const conversationsList = document.querySelector(".md\\:w-80, .w-full.md\\:w-80");
const chatPanel = document.querySelector(".hidden.md\\:flex.flex-1");
function handleMobileChatOpen() {
  if (window.innerWidth < 768) {
    ...
    function lastActiveKey(uid) {
        return `djangou_last_active_${uid}`;
    }

    function setLastActiveIA(uid) {
        localStorage.setItem(lastActiveKey(uid), "ia");
    }

    function wasLastActiveIA(uid) {
        return localStorage.getItem(lastActiveKey(uid)) === "ia";
    }
    ...
    function openIA() {
        ...
        if (currentUser?.uid) setLastActiveIA(currentUser.uid);
